...
// ---------------------------------------------------------------------------
// Full sequence integration — helper to collect events across steps
// ---------------------------------------------------------------------------
...
function runSequence(
  ...
  return steps.flatMap(([eventType, data]) => translate(eventType, data, state))
}

describe("full streaming sequence integration", () => {
  test("reasoning → message sequence produces correct event order", () => {
    const state = makeState()
    const allEvents = runSequence(state, [
      [
        "response.created",
        {
          type: "response.created",
          response: { id: "resp_1", model: "codex" },
        },
      ],
      [
        "response.output_item.added",
    ...
    const allBlockStops = types.filter((t) => t === "content_block_stop")
    expect(allBlockStops.length).toBe(2) // one for thinking, one for text

    expect(types.includes("content_block_delta")).toBe(true)

    expect(types.at(-1)).toBe("message_stop")
    expect(types.at(-2)).toBe("message_delta")

    // Verify block indexes: thinking is 0, text is 1
    const thinkingStart = allEvents.find(
      (e) =>
        e.type === "content_block_start" && e.content_block.type === "thinking",
    )
    const textStart = allEvents.find(
      (e) =>
  ...
  test("function call sequence produces correct events", () => {
    const state = makeState()
    const allEvents = runSequence(state, [
      [
        "response.created",
        {
          type: "response.created",
          response: { id: "resp_fc", model: "codex" },
        },
      ],
      [
        "response.output_item.added",
      ...
      [
        "response.output_item.done",
        {
          type: "response.output_item.done",
          output_index: 0,
          item: { type: "function_call", id: "fc_1", call_id: "call_xyz" },
        },
      ],
      [
        "response.completed",
